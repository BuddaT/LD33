...
public abstract class HeadPart extends Part {
	...
	public void render(GameContainer gc, Graphics g, boolean highlight) {
		...
		if (!isDead())
			g.rotate(200, 80 + getHeight(), rotation);
		g.drawImage(getImage(), 136, 80, c);
		if (!isDead())
			g.rotate(200, 80 + getHeight(), -rotation);
	}
	...
	public void init(GameContainer gc) {
		...
		bounds = new Rectangle(136, 80, getWidth() * 2, getHeight() * 2);
	}
